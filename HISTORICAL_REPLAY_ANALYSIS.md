...
<sup>5</sup> All 9 excluded pairs hit the 60-minute mutation-validation build timeout under parallel build load;
excluded pairs count neither for nor against selection. Both would-miss cases sit on the same commit pair
(`c1184e0c`→`2f03cf41`), whose diff touches only two GitHub Actions workflow files — classified `INERT`
(provably cannot affect any test outcome), so selection correctly ran no tests. `URIOriginTest#testGetInputStream`
and `#testGetInputStrea` (a pre-existing typo in the test name, not a selection bug) failed on head with no
corresponding code change, consistent with environmental flakiness rather than a dependency-tracking miss.
...
<sup>4</sup> The recorded 200-pair run initially skipped 5,808 killing executions: 5,682 from 12
...
<sup>6</sup> 1,011 of 31,729 killing test executions (3.2%) were skipped, spread across 42 commit pairs under
the diff-targeted or whole-tree-fallback rule rather than dependency-match selection. Every one of the 864
killed mutants still had at least one of its killing tests selected — no mutant went entirely uncaught by
selection; the gap is a partial under-selection on the fallback path, not a total blind spot.
